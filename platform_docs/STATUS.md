...
- E11-HOTFIX-UI — Auditoría del builder (commit `5d78fef`): **PASS AFTER FIX** (fix del auditor en `packages/database/src/index.ts`, `apps/api/src/channel-realtime.service.ts`, `apps/api/src/inbox-realtime.service.ts`, `apps/api/src/inbound-webhooks.ts`, `packages/database/src/rbac.integration.ts`, `apps/api/src/tenant-user-management.integration.ts`).
  ...
  - **F1 (pre-existente, corregido)**: la suite `test:security:tenant-isolation` fallaba — 3 archivos de la app (`channel-realtime.service.ts`, `inbox-realtime.service.ts`, `inbound-webhooks.ts`, de E05-S02/E07-S05/E09-S04) importaban `PrismaClient`/`InboundWebhookChannel*`/`Prisma` desde `@whatsapp-platform/database/platform` (ruta privilegiada reservada al composition root). Fix: `import type { Prisma, PrismaClient }` y `InboundWebhookChannel*` exportados ahora como type-only desde el barrel público `@whatsapp-platform/database`; los 3 archivos redirigidos al barrel. Suite completa: 37/37 PASS.
  - **F2 (pre-existente, corregido)**: `rbac.integration.ts` y `tenant-user-management.integration.ts` afirmaban 31 permisos en el catálogo, desactualizados desde que E11-S01 añadió `campaigns.read`/`campaigns.manage` (total real 33, consistente con `packages/rbac/src/index.test.ts`). Aserciones actualizadas a 33; suites user-management (12/12) y rbac de database (11/11) PASS.
  - Lección de proceso registrada: el builder del hotfix no ejecutó ninguna suite de integración de API tras modificar la arquitectura de guards de `contacts.ts` (fix D); las suites contacts (3/3), tenant-isolation (37/37), channel-accounts (16/16), user-management (12/12), theme-engine (11/11), entitlements (5/5) y rbac (11/11) fueron ejecutadas por la auditoría dentro del contenedor.
  - Docker resincronizado: imagen reconstruida con los fixes del auditor, contenedores recreados, todos healthy; smoke HTTP: web 200 en `localhost:3005`, 308 canónico desde `127.0.0.1`, API `/health` 200, preflight login 204.

- E11-HOTFIX-UI — UI Stabilization, Connectivity & Docker Sync: **PASS**.
  - **A. Estilos Tailwind en `/app/ai` y `/app/contacts`**: Se incorporó `@tailwindcss/postcss` con `apps/web/postcss.config.mjs` y la directiva `@import "tailwindcss";` al inicio de `apps/web/app/globals.css`. Turbopack compila con éxito todas las clases utilitarias de Tailwind para las vistas de IA, Contactos y Campañas en 44s.
  - **B. Endpoint de usuarios y roles (`/app/users`)**: Se corrigió `API_BASE_URL` en `tenant-app-users.tsx` para evitar que `process.env.NEXT_PUBLIC_API_BASE_URL ?? ""` evaluara a string vacío `""` (lo que dirigía las peticiones a Next.js en el puerto 3005 devolviendo HTML `<!DOCTYPE...>` y arrojando `SyntaxError: Unexpected token '<'`). Se estandarizó el fallback a `|| "http://localhost:3001"`.
  - **C. Carga de tema (`/app/settings/theme`)**: Se corrigió el fallback de `API_BASE_URL` en `tenant-app-theme-editor.tsx` a `http://localhost:3001` y se aseguró la resolución defensiva de configuración mediante `theme.config ?? defaultTenantBranding()`.
  - **D. Autenticación de contactos (`/app/contacts`)**: En `apps/api/src/contacts.ts`, `@UseGuards(TenantEntitlementGuard)` estaba a nivel de clase, ejecutándose antes de los guards de sesión de nivel método y dejando `request.auth` como `undefined` (provocando 401 Unauthorized inmediato). Se movió dentro del decorador `contactsAuthorized` en el orden canónico: `TenantUserSessionGuard -> TenantContextGuard -> TenantPermissionGuard -> TenantEntitlementGuard`.
  - **E. Temporizador y ciclo de vida QR (`/app/channels`)**: Se corrigió la máquina de estados en `channel-qr-modal.tsx`: el overlay "Código QR expirado" requiere que `qrRaw` exista activamente y haya transcurrido la cuenta regresiva de 30s. Si el QR aún no se ha generado o está en proceso de paring, se muestra el estado de carga ("Generando código QR seguro..."). Al pulsar "Generar nuevo código", se limpia el estado inmediatamente evitando parpadeos al overlay expirado.
  - **Fallbacks globales en frontend**: Se actualizaron todas las referencias de resolución de URL de API en `apps/web` (`layout.tsx`, `page.tsx`, `platform/tenants/`, `channels`, `rules`, `inbox`) para usar `|| "http://localhost:3001"`.
  - **Sincronización Docker**: Imagen `whatsapp-platform-dev:epic00` reconstruida localmente; contenedores recreados y levantados. Ambos endpoints de salud operativos: `http://localhost:3001/health` (200 OK, api) y `http://localhost:3005/health` (200 OK, web).
  - **Verificación**: 167/167 pruebas unitarias de `apps/web` PASS; typecheck monorepo en 18 proyectos 0 errores; Biome en 408 archivos 0 errores.

- E11-S04-patch — Auditoría de login desde `127.0.0.1:3005` (CORS/cookies): **PASS AFTER FIX**.
  - Causa raíz: el navegador estaba en `http://127.0.0.1:3005` pero la web llama a `http://localhost:3001`. Son orígenes distintos para el navegador; la API aplica allowlist CORS de origen exacto (`PLATFORM_WEB_ORIGIN`/`TENANT_WEB_ORIGIN` = `http://localhost:3005`, requisito de SECURITY.md «Origin exacto y CORS explícito») y emitía `Access-Control-Allow-Origin` sólo para `localhost`, rechazando el preflight de `127.0.0.1` (verificado empíricamente con preflights HTTP reales). Ampliar CORS no habría bastado: las cookies de sesión son `SameSite=Strict` y quedan scoped al host `localhost`, por lo que una página servida desde `127.0.0.1` nunca las adjuntaría.
  - Fix: proxy de red de Next.js 16 (`apps/web/proxy.ts`) que canonicaliza el host del loopback — redirige con 308 `127.0.0.1:3005`/`[::1]`/`0.0.0.0` → `localhost:3005` preservando ruta y query, derivando el puerto del header `Host` del cliente (no del bind interno del contenedor 3000). Un solo origen de confianza en lugar de debilitar la política CORS de la API.
  - Verificación: 6 pruebas unitarias nuevas del proxy (167/167 `apps/web` PASS); typecheck monorepo y Biome 0 errores; en contenedores reconstruidos, `GET http://127.0.0.1:3005/app/contacts` responde `308 → http://localhost:3005/app/contacts`, `GET http://localhost:3005/app/contacts` responde 200, y el preflight OPTIONS del login devuelve 204 con `Access-Control-Allow-Origin: http://localhost:3005`.
  - Nota operativa: entrar siempre por `http://localhost:3005`; la IP `127.0.0.1` ahora redirige automáticamente.
...
- E12-S04: el estado de workers es derivado de la disponibilidad de infraestructura (PostgreSQL/Redis); aún no evalúa heartbeats reales de proceso de BullMQ. El hardening de `pingRedis` decodifica contraseñas percent-encoded y rechaza credenciales con CR/LF. Las historias de Action Requests (ex Epic 12 S01–S09) fueron re-homadas como **Epic 27 — Action Requests** en el backlog tras la consolidación de Epic 12 como Reporting/Analytics (auditoría E12-S04); su detalle y priorización siguen pendientes.
