...
- E07-S06 implementa la consola frontend web del Inbox en `apps/web/app/app/inbox/` con arquitectura desacoplada: View Model tipado (`inbox-view-model.ts`), lista de conversaciones (`inbox-thread-list.tsx`) con filtros de estado (`active`, `open`, `pending`, `closed`, `all`), buscador y paginación con cursor; vista de chat y mensajes (`inbox-chat-view.tsx`) con indicadores visuales de entrega (`queued`, `sent`, `delivered`, `read`, `failed`), renderizado seguro de media HTTPS (`referrerPolicy="no-referrer"`), compositor multiline y modal para adjuntar enlaces; y panel lateral de contacto (`inbox-contact-panel.tsx`) con reasignación en vivo de Operador y Unidad Organizacional.
