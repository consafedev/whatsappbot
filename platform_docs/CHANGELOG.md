...
- Auditoría de cuentas: el entorno Compose quedó con 0 Platform Admin, 0 tenants y 0 usuarios tenant. El email y la contraseña candidatos del bootstrap permanecen sólo en `.env` bajo `PLATFORM_ADMIN_BOOTSTRAP_EMAIL` y `PLATFORM_ADMIN_BOOTSTRAP_PASSWORD`, pero el comando explícito `pnpm platform-admin:create` no se ejecutó; aún no son una credencial activa y, al ejecutar el bootstrap, PostgreSQL sólo conservará Argon2id. No hay otros usuarios de prueba persistidos para anunciar.
